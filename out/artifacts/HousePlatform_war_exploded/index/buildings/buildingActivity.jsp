...
<div class="container">
    ...
    <div class="row" ng-repeat="b in activity | limitTo: 3" style="margin-top: 20px;">
        <div class="col-xs-1">
        </div>
        <div class="col-xs-10">
            <div>
                <img src="<%=path %>/{{b.logo}}" class="img-responsive" style="width:975px; height: 300px;">
            </div>
            <div>
                <h3><a target="_blank" href="<%=path %>/activity/activityDetail?activity.id={{b.id}}">{{b.title}}</a></h3>
                <p>{{b.content}}</p>
                <p>活动地址：{{b.address}}</p>
                <p class="text-left">开始时间：{{b.start_time}} &nbsp;&nbsp;结束时间：{{b.end_time}}</p>
            </div>
        </div>
        <div class="col-xs-1">
        </div>
    ...
    (function ($) {
        ...
        app.controller('myCtrl', function ($scope, $http, $timeout) {
            ...
            function extracted(x) {
                $http({
                    method: 'POST',
                    url: "<%=path%>/activity/navActivity?buildingsId=" + buildingsId,
                    data: {"pageNo": x}
                }).then(function successCallback(response) {
                    $scope.total = response.data.pager.total;
                    $scope.totalPageSize = response.data.pager.totalPageSize;
                    $scope.activity = response.data.pager.result;
