...
<div class="container">
    ...
    <div class="row" id="content" style="margin-top:20px;">
        ...
        <div class="col-xs-10">
            <form id="from">
                <textarea class="form-control" rows="3"  name="message.content" style="height: 200px;" placeholder="请输入您的看法......"></textarea>
                <input type="hidden" name="userPrice.id" id="user" value="${user.id}"/>
                <input style="margin-top: 10px;" type="button" class="btn btn-default" value="发表留言" ng-click="add();"/>
            </form>
            <div  style="border: 1px solid #e7e7e7; margin-top: 20px;border-radius:5px 5px 5px 5px;" ng-repeat="a in comment">
                <div class="media-left">
                    <a href="#">
                        <img class="media-object"  alt="80x80" src="<%=path%>/{{a.userPrice.headicon}}" data-holder-rendered="true" style="width: 64px; height: 64px;">
                    </a>
                </div>
                <div class="media-body">
                    <h4 class="media-heading" ></h4>
                    <p>{{a.content}}</p>
                    <p>{{a.leave_time | date:'yyyy-MM-dd HH:mm'}}</p>
                </div>
            </div>
         </div>
    </div>
        ...
        <div class="col-sm-4">
            <nav aria-label="Page navigation" class="pull-center">
                <ul class="pagination">
                    ...
                    <li><a class="page" ng-show="count1>=5 && (totalPageSize-4) <= count1"
                           ng-click="page(count1-4)">{{count1-4}}</a></li>
                    <li><a class="page" ng-show="count1>=4 && (totalPageSize-3) <= count1"
                           ng-click="page(count1-3)">{{count1-3}}</a></li>
                    <li><a class="page" ng-show="count1>=3 && (totalPageSize-2) <= count1"
                           ng-click="page(count1-2)">{{count1-2}}</a></li>
                    <li><a class="page" ng-show="count1>=2 "
    ...
    (function ($) {
        ...
        app.controller('myCtrl', function ($scope, $http, $timeout) {
            ...
            function extracted(x) {
                $http({
                    method: 'POST',
                    url: "<%=path%>/message/buildingsMessage?buildingsId=" + buildingsId,
                    data: {"page": x}
                }).then(function successCallback(response) {
                    $scope.total = response.data.messagePager.total;
                    $scope.totalPageSize = response.data.messagePager.totalPageSize;
                    $scope.comment = response.data.messagePager.result;
            ...
            $scope.add = function () {
                ...
                $.post("<%=path %>/message/addMessage?buildingsPrice.id=" + buildingsId,
                    $("#from").serialize(),
                    function(data){
